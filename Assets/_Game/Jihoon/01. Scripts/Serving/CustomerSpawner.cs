...
public class CustomerSpawner : MonoBehaviour
{
    ...
    private bool TrySpawnOne()
    {
        ...
        if (!spot.TrySeatCustomer(prefab, NextPatience()))
        {
            return false;
        }
    ...
    /// <summary>Patience for the next customer, shortened the same way.</summary>
    private float NextPatience()
    {
        float ramp = Mathf.Pow(patienceDecay, SpawnedCount);
        float raw = UnityEngine.Random.Range(patienceRange.x, patienceRange.y);
        return Mathf.Max(minPatience, raw * ramp);
    }
